...
public interface CommandeADomicileRepository extends JpaRepository<CommandeADomicile,Long>{
	...
	@Query("select distinct s from CommandeADomicile s left join fetch s.itemsMenu where s.client.id = ?1")
	List<CommandeADomicile> findByClientWithItemsMenu(Long idClient);
}
